...
const emptyItemForm = { actionCode: '', actionName: '', unitOfMeasure: 'request', baseCreditCost: 0, feeCreditCost: 0, feeCategoryId: '' }
...
export default function RateCardDetail() {
  ...
  const { data: card, loading, error, refetch } = useApi(() => api.get(`/rate-cards/${id}`), [id])
  const { data: items, refetch: refetchItems } = useApi(() => api.get(`/rate-cards/${id}/items`), [id])
  const { data: feeCategories } = useApi(() => api.get('/fee-categories'))
  ...
  const setField = (field) => (e) => setItemForm((f) => ({ ...f, [field]: e.target.value }))
  ...
  const feeCatMap = Object.fromEntries((feeCategories || []).map((fc) => [fc.id, fc]))
  const feeCatOptions = (feeCategories || []).map((fc) => ({ value: fc.id, label: `${fc.code} — ${fc.name}` }))
                  ...
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    {isEditable && (
                      <td className="px-4 py-3 text-sm text-right">
                        <button onClick={() => openEditItem(item)} className="text-blue-600 hover:underline text-xs mr-3">编辑</button>
                        <button onClick={() => setDeleteTarget(item)} className="text-red-500 hover:underline text-xs">删除</button>
                      </td>
                    )}
                  </tr>
                )
