...
const emptyForm = { name: '', code: '', isRevenueBearing: true, accountingTreatment: '', defaultExpiryDays: 365 }
...
const defaultColors = { border: 'border-l-gray-400', bg: 'bg-gray-50', text: 'text-gray-700', badge: 'bg-gray-100 text-gray-700', icon: '📋' }
...
function TypeCard({ gt, onEdit, onDelete }) {
    ...
    <div className={`bg-white rounded-lg shadow border-l-4 ${colors.border} hover:shadow-md transition-shadow`}>
      ...
      <div className="border-t border-gray-100 px-5 py-2.5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => onEdit(gt)} className="text-xs text-blue-600 hover:text-blue-800">编辑</button>
          <button onClick={() => onDelete(gt)} className="text-xs text-red-500 hover:text-red-700">删除</button>
        </div>
      </div>
    </div>
  )
