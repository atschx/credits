...
import type { DashboardStats, MonthlyRevenue, AccountRevenue } from '../types'
...
function MiniBar({ data, color = '#3b82f6' }: MiniBarProps) {
...
function TrendChart({ data }: TrendChartProps) {
  if (!data || data.length === 0) {
    return <div className="text-gray-400 text-sm text-center py-8">暂无收入数据 No revenue data</div>
  }
...
export default function Dashboard() {
  ...
  const accountRevenues: AccountRevenue[] = stats?.accountRevenue || []
  const monthlyRevenues: MonthlyRevenue[] = stats?.monthlyRevenue || []

  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 mb-1">仪表盘 Dashboard</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white rounded-lg shadow p-5">
          ...
          <p className="text-xs text-gray-400 mb-4">已确认 vs 递延收入 Recognized vs Deferred</p>
          {loading || !stats ? (
            <div className="h-40 bg-gray-50 rounded animate-pulse" />
          ) : (() => {
            const recognized: number = Number(stats.recognizedRevenue) || 0
            const deferred: number = Number(stats.deferredRevenue) || 0
            const total: number = recognized + deferred || 1
            const recPct: string = ((recognized / total) * 100).toFixed(1)
            const defPct: string = ((deferred / total) * 100).toFixed(1)
            return (
              <div>
      ...
      <div className="bg-white rounded-lg shadow mb-6">
        ...
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-400 text-sm">暂无消耗数据 No consumption data</div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50">
                <th className="px-5 py-2.5 text-left text-xs font-medium text-gray-500 uppercase">排名 Rank</th>
                ...
                <th className="px-5 py-2.5 text-left text-xs font-medium text-gray-500 uppercase">占比 Share</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {accountRevenues.map((ar, i) => {
                const totalRev: number = Number(stats!.recognizedRevenue) || 1
                const pct: string = ((Number(ar.recognizedRevenue) / totalRev) * 100).toFixed(1)
                return (
