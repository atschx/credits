            ...
            return (
              <div key={cat.id} className="bg-white rounded-lg shadow hover:shadow-md transition-shadow">
                ...
                <div className="border-t border-gray-100 px-5 py-2.5 flex items-center justify-end gap-3">
                  <button onClick={() => openEdit(cat)} className="text-xs text-blue-600 hover:text-blue-800">编辑</button>
                  <button onClick={() => setDeleteTarget(cat)} className="text-xs text-red-500 hover:text-red-700">删除</button>
                </div>
              </div>
            )
          })}
