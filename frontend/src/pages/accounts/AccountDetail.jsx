...
function BalanceBar({ purchased, promotional, bonus, total }) {
...
function GrantCard({ grant, grantTypes }) {
...
const emptyGrantForm = { grantTypeCode: '', amount: '', costBasisPerUnit: '', currency: 'USD', sourceReference: '', expiryDays: '' }
const LARGE_AMOUNT_THRESHOLD = 50000

export default function AccountDetail() {
  const { id } = useParams()
  const { data: account, loading, error, refetch: refetchAccount } = useApi(() => api.get(`/accounts/${id}`), [id])
  const { data: balance, refetch: refetchBalance } = useApi(() => api.get(`/accounts/${id}/balance`), [id])
  const { data: grants, refetch: refetchGrants } = useApi(() => api.get('/credits/grants', { params: { accountId: id } }), [id])
  const { data: grantTypes } = useApi(() => api.get('/grant-types'))
  const { data: rateCards } = useApi(() => api.get('/rate-cards'))

  // Grant modal
  const [grantModal, setGrantModal] = useState(false)
  const [grantForm, setGrantForm] = useState(emptyGrantForm)
  const [grantStep, setGrantStep] = useState('form') // 'form' | 'confirm'
  ...
  const lastRechargeGrant = allGrants.length > 0
    ? allGrants.reduce((latest, g) => (!latest || new Date(g.createdAt) > new Date(latest.createdAt)) ? g : latest, null)
    : null
